function k_est = reconstruction2(f,q,name) 
tic;
load('parameters_k.mat');
load('PINN.mat');
load(name);

Swmeanerr(1:end-1) = Swmeanerr(1:end-1)./(1+normrnd(0,0.03,[length(Swmeanerr)-1 1]));
Swmeanerr(Swmeanerr>0.999) = 0.999;
